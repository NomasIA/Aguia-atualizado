...
import { Wrench, Plus, Calendar, DollarSign, CheckCircle, Undo2, Package } from 'lucide-react';
...
export default function MaquinariosPage() {
  ...
  const marcarRecebido = async (contratoId: string) => {
    try {
      ...
      toast({ title: 'Sucesso', description: 'Recebimento confirmado. Entrada criada e quantidade devolvida.' });
  ...
  const desfazerRecebimento = async (contratoId: string) => {
    try {
      const { error } = await supabase.rpc('desfazer_recebimento_locacao', { p_contrato_id: contratoId });
      if (error) throw error;

      toast({ title: 'Sucesso', description: 'Recebimento desfeito.' });
      window.dispatchEvent(new Event('kpi-refresh'));
      loadData();
    } catch (error: any) {
      toast({ title: 'Erro', description: error.message, variant: 'destructive' });
  ...
  const getStatusBadge = (maquina: Maquina) => {
    if (maquina.quantidade_disponivel === 0) {
      return <span className="px-2 py-1 rounded-md text-xs font-medium border bg-orange-500/10 text-orange-500 border-orange-500/20">Locado ({maquina.quantidade_disponivel}/{maquina.quantidade})</span>;
  ...
  const maquinasDisponiveis = maquinas.filter(m => m.quantidade_disponivel > 0);
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {maquinasDisponiveis.length === 0 ? (
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-muted text-lg">Nenhum equipamento disponível</p>
              </div>
            ) : (
                ...
                <Card key={maquina.id} className="card">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="pt-2 border-t border-border">
                      ...
                      <Button onClick={() => openSimulation(maquina)} className="w-full btn-primary">
                        ...
                        <tr key={contrato.id} className="border-b border-border/50 hover:bg-accent/5">
                          ...
                          <td className="p-3 text-center">
                            {contrato.recebido ? (
                              <div className="text-xs">
                                ...
                                {contrato.data_recebimento && <p className="text-muted mt-1">{format(new Date(contrato.data_recebimento), 'dd/MM/yyyy')}</p>}
                              </div>
                            ) : (
                          ...
                          <td className="p-3 text-center">
                            {!contrato.recebido ? (
                              ...
                              <Button size="sm" variant="ghost" onClick={() => desfazerRecebimento(contrato.id)} className="text-xs hover:bg-orange-500/10">
